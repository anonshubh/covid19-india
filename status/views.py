...
class HomeView(View):
    def get(self,request,*args,**kwargs):
        ...
        comb = []
        for i in range(38):
            data = zip(combined[i][0],combined[i][1],combined[i][2],combined[i][3],combined[i][4],combined[i][5],combined[i][6],combined[i][7],combined[i][8],combined[i][9],combined[i][10])
            comb.append(data)
        context = {
            'combined':data
        }
